...
describe('Odo Responsive Images', () => {
  ...
  function getFixtureClone(id) {
    const markup = window.__html__[`fixtures/${id}.html`];
  ...
  describe('basic fixture', () => {
    ...
    it('can add an array of images', () => {
      const image1 = getFixtureClone('basic');
    ...
    it('can clear all current images', () => {
      const bgImage = getFixtureClone('background');
      document.body.appendChild(bgImage);
      OdoResponsiveImages.add([bgImage]);
  ...
  describe('custom fixture', () => {
    ...
    beforeEach(() => {
      image = getFixtureClone('basic');
      document.body.appendChild(image);
      img = image.querySelector('img');
      img.testProperty = 'foo';

      OdoResponsiveImages.add(image);
    });
    ...
    it('should not break the reference to children', () => {
      ...
      OdoResponsiveImages.load(image);

      const picture = getLastResponsiveImage();
      const imgElement = picture.querySelector('img');

      expect(document.body.contains(img)).to.be.true;
      expect(imgElement.testProperty).to.equal(img.testProperty);
    });
  });

  describe('background fixture', () => {
    beforeEach(() => {
      createFixture('background');
    });
  ...
  describe('srcset background fixture', () => {
    beforeEach(() => {
      createFixture('srcset-background');
    });
